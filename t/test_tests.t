...
send_it(make_msg(['https://www.google.com','http://www.alibaba.com','https://www.cpan.org','https://wikipedia.com','https://yandex.ru/'],'http'));
test_test(title => 'Validates 5 mixed links', skip_out => 1);
...
test_test(title => 'Fails for invalid uris', skip_err => 1);
...
test_test(title => 'Fails for 404 url', skip_err => 1);
...
test_test(title => 'Fails for invalid name or service', skip_err => 1);
...
test_test(title => "Fails for page with title containing 'error'", skip_err => 1);
...
test_test(title => 'Fails when wrong content type for image', skip_err => 1);
