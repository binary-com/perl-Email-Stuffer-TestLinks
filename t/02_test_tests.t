...
test_out("ok 1 - it died");
dies_ok { Email::Stuffer->from('sender@example.com')->send_or_die(); }
"it died";
test_test("send_or_die can still die");

test_out("ok 1 - sent email ok");
lives_ok { send_it( make_msg(@urls) ) } "sent email ok";
test_test("Email with no links");
...
@urls = (
    'https://www.google.com', 'http://www.alibaba.com',
    'www.cpan.org',           'wikipedia.com',
    'https://yandex.ru/'
);
test_out( make_teststr(@urls) );
send_it( make_msg(@urls) );
...
send_it( make_msg(@urls) );
test_test( title => "Fails for 404 link (@urls)", skip_err => 1 );

@urls = ('http://site.invalid');
test_out("not ok 1 - Link in email works ($urls[0])");
send_it( make_msg(@urls) );
test_test( title => "Fails for bad host (@urls)", skip_err => 1 );

@urls = ('@!*%/&');
test_out("not ok 1 - Link in email works ($urls[0])");
send_it( make_msg(@urls) );
test_test( title => "Fails for garbage url (@urls)", skip_err => 1 );
